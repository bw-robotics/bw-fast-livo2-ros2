                               ...
                               const int pyramid_level, const int halfpatch_size, float *patch)
{
  ...
  {
    if (verbose_logging_) printf("Affine warp is NaN, probably camera has no translation\n");
    return;
  }
...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  // "<<t_5<<endl;
  last_retrieved_points_ = total_points;
  if (verbose_logging_) printf("[ VIO ] Retrieve %d points from visual sparse map\n", total_points);
}
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  last_added_points_ = add;
  if (verbose_logging_) printf("[ VIO ] Append %d new visual map points\n", add);
  // printf("pg.size: %d \n", pg.size());
  // printf("B1. : %.6lf \n", t_b1);
  // printf("B2. : %.6lf \n", t_b2);
}
...
void VIOManager::updateVisualMapPoints(cv::Mat img)
{
  ...
  last_updated_points_ = update_num;
  if (verbose_logging_) printf("[ VIO ] Update %d points in visual submap\n", update_num);
}
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  last_total_ms_        = (t7 - t1 - (t5 - t4)) * 1000.0;
