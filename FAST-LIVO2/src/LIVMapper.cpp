...
void LIVMapper::handleFirstFrame() 
{
  if (!is_first_frame)
  {
    ...
    // Publish static transform after first frame
    publishStaticTransform();
  }
}

void LIVMapper::publishStaticTransform()
{
  if (static_transform_published_) return;
  
  geometry_msgs::msg::TransformStamped transform_stamped;
  
  transform_stamped.header.frame_id = "map";
  transform_stamped.child_frame_id = "camera_init";
  
  if (tf_buffer_->canTransform("map", "base_link", rclcpp::Time(0), rclcpp::Duration::from_seconds(0.1))) {
    try {
      auto tf_base_link_to_map = tf_buffer_->lookupTransform("map", "base_link", rclcpp::Time(0),
                                                              rclcpp::Duration::from_seconds(0.1));
      
      transform_stamped.header.stamp = this->node->now();
      transform_stamped.transform.translation = tf_base_link_to_map.transform.translation;
      transform_stamped.transform.rotation = tf_base_link_to_map.transform.rotation;
      static_tf_broadcaster_->sendTransform(transform_stamped);
      static_transform_published_ = true;
      RCLCPP_INFO(this->node->get_logger(), "Published static transform from map to camera_init");
    } catch (const tf2::TransformException &ex) {
      RCLCPP_ERROR(this->node->get_logger(), "tf error during lookup: %s", ex.what());
    }
  } else {
    RCLCPP_ERROR(this->node->get_logger(), "tf error - couldn't find transform during the expected timeframe");
  }
}
...
cv::Mat LIVMapper::getImageFromMsg(const sensor_msgs::msg::Image::ConstSharedPtr &img_msg)
{
  ...
  try {
    ...
    if (cv_ptr == nullptr) {
      RCLCPP_ERROR(this->node->get_logger(), "cv_bridge::toCvShare returned nullptr");
      return cv::Mat();
    }
    img = cv_ptr->image.clone(); // Make a deep copy to avoid issues
    if (img.empty()) {
      RCLCPP_ERROR(this->node->get_logger(), "Converted image is empty");
      return cv::Mat();
    }
    ...
    RCLCPP_ERROR(this->node->get_logger(), "Exception in getImageFromMsg: %s", e.what());
    return cv::Mat();
  }
  return img;
}
...
void LIVMapper::publish_img_rgb(const image_transport::Publisher &pubImage, VIOManagerPtr vio_manager)
{
  ...
  out_msg.header.stamp = frameStamp();
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  laserCloudmsg.header.stamp = frameStamp();
...
void LIVMapper::publish_effect_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudEffect, const std::vector<PointToPlane> &ptpl_list)
{
  ...
  laserCloudFullRes3.header.stamp = frameStamp();
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
  ...
  odomAftMapped.header.stamp = frameStamp();
...
{
  msg_body_pose.header.stamp = frameStamp();
